...
mat4f CalcModelMatrix(vec3f translate, vec3f rotate, vec3f scale)
{
	mat4f scaleMatrix = mat4f(scale.x, 0, 0, 0,
		0, scale.y, 0, 0,
		0, 0, scale.z, 0,
		0, 0, 0, 1);

	return mat4f::translate(translate) *
		mat4f::rotatex(rotate.x) * mat4f::rotatez(rotate.z) * mat4f::rotatey(rotate.y) *
		scaleMatrix;
}
...
{
	float maxXVal = -MAXNUM;
	float minXVal = MAXNUM;
	float maxYVal = -MAXNUM;
	float minYVal = MAXNUM;
	float maxZVal = -MAXNUM;
	float minZVal = MAXNUM;
...
vector<ObjMesh>::iterator ObjMesh::GetObjMeshWithMID(vector<ObjMesh> &objMeshes, const char *MID)
{
